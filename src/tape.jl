...
function record!(
    tp::InstructionTape, ::Type{InstructionType}, args...
...
function Base.:(==)(a::AbstractInstruction, b::AbstractInstruction)
    return (
        a.func == b.func && a.input == b.input && a.output == b.output && a.cache == b.cache
    )
end
...
function compactrepr(x::AbstractArray)
    return length(x) < 5 ? match(r"\[.*?\]", repr(x)).match : summary(x)
end
...
function Base.show(io::IO, instruction::AbstractInstruction, pad="")
    ...
    return print(io, pad, "  cache:  ", compactrepr(instruction.cache))
end

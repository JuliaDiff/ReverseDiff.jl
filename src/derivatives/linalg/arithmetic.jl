...
for TV in (:AbstractVector, :Vector)
    @eval begin
        ...
        Base.:*(A::Adjoint{<:TrackedReal, <:TrackedVector{T}}, B::$TV{T}) where {T <: Real} = dot(A, B)
        Base.:*(A::Adjoint{<:TrackedReal, <:$TV{T}}, B::TrackedVector{T}) where {T <: Real} = dot(A, B)
        Base.:*(A::Transpose{<:TrackedReal, <:TrackedVector{T}}, B::$TV{T}) where {T <: Real} = dot(A, B)
        Base.:*(A::Transpose{<:TrackedReal, <:$TV{T}}, B::TrackedVector{T}) where {T <: Real} = dot(A, B)
...
Base.:*(A::Transpose{<:TrackedReal, <:TrackedVector{<:Real, D}}, B::TrackedVector{<:Real, D}) where {D} = dot(parent(A), B)

...
for g in (:map, :broadcast), f in SKIPPED_BINARY_SCALAR_FUNCS
    @eval begin
        @inline Base.$(g)(f::typeof($f), x::TrackedArray, y::TrackedArray) =
            $(g)(SkipOptimize(f), x, y)
        @inline Base.$(g)(f::typeof($f), x::TrackedArray, y::TrackedReal) =
            $(g)(SkipOptimize(f), x, y)
        @inline Base.$(g)(f::typeof($f), x::TrackedReal, y::TrackedArray) =
            $(g)(SkipOptimize(f), x, y)
    end
    for A in ARRAY_TYPES
        @eval begin
            @inline Base.$(g)(f::typeof($f), x::$A, y::TrackedArray) =
                $(g)(SkipOptimize(f), x, y)
            @inline Base.$(g)(f::typeof($f), x::TrackedArray, y::$A) =
                $(g)(SkipOptimize(f), x, y)
            @inline Base.$(g)(f::typeof($f), x::$A, y::TrackedReal) =
                $(g)(SkipOptimize(f), x, y)
            @inline Base.$(g)(f::typeof($f), x::TrackedReal, y::$A) =
                $(g)(SkipOptimize(f), x, y)
        end
    end
    for R in REAL_TYPES
        @eval begin
            @inline Base.$(g)(f::typeof($f), x::$R, y::TrackedArray) =
                $(g)(SkipOptimize(f), x, y)
            @inline Base.$(g)(f::typeof($f), x::TrackedArray, y::$R) =
                $(g)(SkipOptimize(f), x, y)
        end
    end
end
...
for g in (:map, :broadcast)
    @eval @inline Base.$(g)(f::SkipOptimize{F}, t::TrackedArray) where {F} =
        $(g)(f.f, value(t))
end

for g in (:map, :broadcast)
    @eval begin
        @inline Base.$(g)(f::SkipOptimize{F}, x::TrackedArray, y::TrackedArray) where {F} =
            $(g)(f.f, value(x), value(y))
        @inline Base.$(g)(f::SkipOptimize{F}, x::TrackedArray, y::TrackedReal) where {F} =
            $(g)(f.f, value(x), value(y))
        @inline Base.$(g)(f::SkipOptimize{F}, x::TrackedReal, y::TrackedArray) where {F} =
            $(g)(f.f, value(x), value(y))
    end
    for A in ARRAY_TYPES
        @eval begin
            @inline Base.$(g)(f::SkipOptimize{F}, x::$A, y::TrackedArray) where {F} =
                $(g)(f.f, value(x), value(y))
            @inline Base.$(g)(f::SkipOptimize{F}, x::TrackedArray, y::$A) where {F} =
                $(g)(f.f, value(x), value(y))
            @inline Base.$(g)(f::SkipOptimize{F}, x::$A, y::TrackedReal) where {F} =
                $(g)(f.f, value(x), value(y))
            @inline Base.$(g)(f::SkipOptimize{F}, x::TrackedReal, y::$A) where {F} =
                $(g)(f.f, value(x), value(y))
        end
    end
    for R in REAL_TYPES
        @eval begin
            @inline Base.$(g)(f::SkipOptimize{F}, x::$R, y::TrackedArray) where {F} =
                $(g)(f.f, value(x), value(y))
            @inline Base.$(g)(f::SkipOptimize{F}, x::TrackedArray, y::$R) where {F} =
                $(g)(f.f, value(x), value(y))
        end
    end
end
...
for g! in (:map!, :broadcast!),
    (M, f, arity) in DiffRules.diffrules(; filter_modules=nothing)
    ...
    if arity == 1
        @eval @inline Base.$(g!)(f::typeof($M.$f), out::TrackedArray, t::TrackedArray) =
            $(g!)(ForwardOptimize(f), out, t)
    elseif arity == 2
        @eval begin
            @inline Base.$(g!)(
                f::typeof($M.$f), out::TrackedArray, x::TrackedArray, y::TrackedArray
            ) = $(g!)(ForwardOptimize(f), out, x, y)
            @inline Base.$(g!)(
                f::typeof($M.$f), out::TrackedArray, x::TrackedArray, y::TrackedReal
            ) = $(g!)(ForwardOptimize(f), out, x, y)
            @inline Base.$(g!)(
                f::typeof($M.$f), out::TrackedArray, x::TrackedReal, y::TrackedArray
            ) = $(g!)(ForwardOptimize(f), out, x, y)
        end
        for A in ARRAY_TYPES
            @eval begin
                @inline Base.$(g!)(
                    f::typeof($M.$f), out::TrackedArray, x::$A, y::TrackedArray
                ) = $(g!)(ForwardOptimize(f), out, x, y)
                @inline Base.$(g!)(
                    f::typeof($M.$f), out::TrackedArray, x::TrackedArray, y::$A
                ) = $(g!)(ForwardOptimize(f), out, x, y)
                @inline Base.$(g!)(
                    f::typeof($M.$f), out::TrackedArray, x::$A, y::TrackedReal
                ) = $(g!)(ForwardOptimize(f), out, x, y)
                @inline Base.$(g!)(
                    f::typeof($M.$f), out::TrackedArray, x::TrackedReal, y::$A
                ) = $(g!)(ForwardOptimize(f), out, x, y)
            end
        end
        for R in REAL_TYPES
            @eval begin
                @inline Base.$(g!)(
                    f::typeof($M.$f), out::TrackedArray, x::$R, y::TrackedArray
                ) = $(g!)(ForwardOptimize(f), out, x, y)
                @inline Base.$(g!)(
                    f::typeof($M.$f), out::TrackedArray, x::TrackedArray, y::$R
                ) = $(g!)(ForwardOptimize(f), out, x, y)
            end
        end
    end
...
for (g!, g) in ((:map!, :map), (:broadcast!, :broadcast))
    @eval function Base.$(g!)(
        f::ForwardOptimize{F}, out::TrackedArray{S}, x::TrackedArray{X}
    ) where {F,S,X}
    ...
    for TX in (:TrackedArray, :TrackedReal), TY in (:TrackedArray, :TrackedReal)
        ...
        @eval function Base.$(g!)(
            f::ForwardOptimize{F}, out::TrackedArray{S}, x::$(TX){X}, y::$(TY){Y}
        ) where {F,S,X,Y}
            result = DiffResults.GradientResult(SVector(zero(S), zero(S)))
            df =
                (vx, vy) ->
                    ForwardDiff.gradient!(result, s -> f.f(s[1], s[2]), SVector(vx, vy))
    ...
    for A in ARRAY_TYPES
        @eval function Base.$(g!)(
            f::ForwardOptimize{F}, out::TrackedArray, x::TrackedReal{X,D}, y::$A
        ) where {F,X,D}
        ...
        @eval function Base.$(g!)(
            f::ForwardOptimize{F}, out::TrackedArray, x::$A, y::TrackedReal{Y,D}
        ) where {F,Y,D}
        ...
        @eval function Base.$(g!)(
            f::ForwardOptimize{F}, out::TrackedArray, x::TrackedArray{X}, y::$A
        ) where {F,X}
        ...
        @eval function Base.$(g!)(
            f::ForwardOptimize{F}, out::TrackedArray, x::$A, y::TrackedArray{Y}
        ) where {F,Y}
...
for R in REAL_TYPES
    @eval begin
        @inline Base.broadcast!(
            f::ForwardOptimize{F}, out::TrackedArray, x::TrackedArray, y::$R
        ) where {F} = broadcast!(ForwardOptimize(t -> f.f(t, y)), out, x)
        @inline Base.broadcast!(
            f::ForwardOptimize{F}, out::TrackedArray, x::$R, y::TrackedArray
        ) where {F} = broadcast!(ForwardOptimize(t -> f.f(x, t)), out, y)
    end
end
...
for g in (:map, :broadcast), (M, f, arity) in DiffRules.diffrules(; filter_modules=nothing)
    ...
    if arity == 1
        @eval @inline Base.$(g)(f::typeof($M.$f), t::TrackedArray) =
            $(g)(ForwardOptimize(f), t)
    elseif arity == 2
        (M, f) in SKIPPED_DIFFRULES && continue
        ...
        @eval begin
            @inline Base.$(g)(f::typeof($M.$f), x::TrackedArray, y::TrackedArray) =
                $(g)(ForwardOptimize(f), x, y)
            @inline Base.$(g)(f::typeof($M.$f), x::TrackedArray, y::TrackedReal) =
                $(g)(ForwardOptimize(f), x, y)
            @inline Base.$(g)(f::typeof($M.$f), x::TrackedReal, y::TrackedArray) =
                $(g)(ForwardOptimize(f), x, y)
        end
        for A in ARRAY_TYPES
            @eval begin
                @inline Base.$(g)(f::typeof($M.$f), x::$A, y::TrackedArray) =
                    $(g)(ForwardOptimize(f), x, y)
                @inline Base.$(g)(f::typeof($M.$f), x::TrackedArray, y::$A) =
                    $(g)(ForwardOptimize(f), x, y)
                @inline Base.$(g)(f::typeof($M.$f), x::$A, y::TrackedReal) =
                    $(g)(ForwardOptimize(f), x, y)
                @inline Base.$(g)(f::typeof($M.$f), x::TrackedReal, y::$A) =
                    $(g)(ForwardOptimize(f), x, y)
            end
        end
        for R in REAL_TYPES
            @eval begin
                @inline Base.$(g)(f::typeof($M.$f), x::$R, y::TrackedArray) =
                    $(g)(ForwardOptimize(f), x, y)
                @inline Base.$(g)(f::typeof($M.$f), x::TrackedArray, y::$R) =
                    $(g)(ForwardOptimize(f), x, y)
            end
        end
    end
...
for g in (:map, :broadcast)
    ...
    for A in ARRAY_TYPES
        @eval function Base.$(g)(
            f::ForwardOptimize{F}, x::TrackedReal{X,D}, y::$A
        ) where {F,X,D}
            result = DiffResults.DiffResult(zero(X), zero(D))
            df = let result = result
                (vx, vy) -> let vy = vy
        ...
        @eval function Base.$(g)(
            f::ForwardOptimize{F}, x::$A, y::TrackedReal{Y,D}
        ) where {F,Y,D}
            result = DiffResults.DiffResult(zero(Y), zero(D))
            df = let result = result
                (vx, vy) -> let vx = vx
        ...
        @eval function Base.$(g)(
            f::ForwardOptimize{F}, x::TrackedArray{X,D}, y::$A
        ) where {F,X,D}
            result = DiffResults.GradientResult(SVector(zero(X)))
            df =
                (vx, vy) -> let vy = vy
        ...
        @eval function Base.$(g)(
            f::ForwardOptimize{F}, x::$A, y::TrackedArray{Y,D}
        ) where {F,Y,D}
            result = DiffResults.GradientResult(SVector(zero(Y)))
            df =
                (vx, vy) -> let vx = vx
    ...
    for TX in (:TrackedArray, :TrackedReal), TY in (:TrackedArray, :TrackedReal)
        ...
        @eval function Base.$(g)(
            f::ForwardOptimize{F}, x::$(TX){X,D}, y::$(TY){Y,D}
        ) where {F,X,Y,D}
...
for R in REAL_TYPES
    @eval begin
        @inline Base.broadcast(
            f::ForwardOptimize{F}, x::TrackedArray{X,D}, y::$R
        ) where {F,X,D} = broadcast(ForwardOptimize(t -> f.f(t, y)), x)
        @inline Base.broadcast(
            f::ForwardOptimize{F}, x::$R, y::TrackedArray{Y,D}
        ) where {F,Y,D} = broadcast(ForwardOptimize(t -> f.f(x, t)), y)
    end
end
...
for (g!, g) in ((:map!, :map), (:broadcast!, :broadcast))
    @eval begin
        @noinline function special_forward_exec!(
            instruction::SpecialInstruction{typeof($g)}
        )
...
for (F, broadcast_f) in (
    ...
    @eval begin
        @inline Base.broadcast(
            ::$F, x::TrackedArray{X,D}, y::TrackedArray{Y,D}
        ) where {X,Y,D} = $(broadcast_f)(x, y, D)
        @inline Base.broadcast(
            ::$F, x::TrackedReal{X,D}, y::TrackedArray{Y,D}
        ) where {X,Y,D} = $(broadcast_f)(x, y, D)
        @inline Base.broadcast(
            ::$F, x::TrackedArray{X,D}, y::TrackedReal{Y,D}
        ) where {X,Y,D} = $(broadcast_f)(x, y, D)
    end
    for A in ARRAY_TYPES
        @eval begin
            @inline Base.broadcast(::$F, x::TrackedArray{X,D}, y::$A{<:Real}) where {X,D} =
                $(broadcast_f)(x, y, D)
            @inline Base.broadcast(::$F, x::$A{<:Real}, y::TrackedArray{Y,D}) where {Y,D} =
                $(broadcast_f)(x, y, D)
            @inline Base.broadcast(::$F, x::TrackedReal{X,D}, y::$A{<:Real}) where {X,D} =
                $(broadcast_f)(x, y, D)
            @inline Base.broadcast(::$F, x::$A{<:Real}, y::TrackedReal{Y,D}) where {Y,D} =
                $(broadcast_f)(x, y, D)
        end
    end
    for R in REAL_TYPES
        @eval begin
            @inline Base.broadcast(::$F, x::TrackedArray{X,D}, y::$R) where {X,D} =
                $(broadcast_f)(x, y, D)
            @inline Base.broadcast(::$F, x::$R, y::TrackedArray{Y,D}) where {Y,D} =
                $(broadcast_f)(x, y, D)
        end
    end
end
...
@noinline function special_forward_exec!(
    instruction::SpecialInstruction{Tuple{typeof(broadcast),typeof(+)}}
)
...
@noinline function special_reverse_exec!(
    ...
    output = instruction.output
    output_deriv = deriv(output)
    n_partials, d_partials, a_bound, b_bound, n_partials_bound, d_partials_bound =
        instruction.cache
    istracked(a) &&
        broadcast_increment_deriv!(a, output_deriv, n_partials, a_bound, n_partials_bound)
    istracked(b) &&
        broadcast_increment_deriv!(b, output_deriv, d_partials, b_bound, d_partials_bound)
    unseed!(output)
    return nothing
end
...
@noinline function special_reverse_exec!(
    ...
    n_partials, d_partials, a_bound, b_bound, n_partials_bound, d_partials_bound =
        instruction.cache
    istracked(a) &&
        broadcast_increment_deriv!(a, output_deriv, d_partials, a_bound, d_partials_bound)
    istracked(b) &&
        broadcast_increment_deriv!(b, output_deriv, n_partials, b_bound, n_partials_bound)
    unseed!(output)
    return nothing
end
...
function broadcast_pow(x, y, ::Type{D}) where {D}
    ...
    cache = (
        ...
        index_bound(x, out),
        index_bound(y, out),
        index_bound(bs_partials, out),
        index_bound(ex_partials, out),
    )
    record!(tp, SpecialInstruction, (broadcast, ^), (x, y), out, cache)
    return out
end
...
@noinline function special_reverse_exec!(
    ...
    output = instruction.output
    output_deriv = deriv(output)
    bs_partials, ex_partials, a_bound, b_bound, bs_partials_bound, ex_partials_bound =
        instruction.cache
    istracked(a) &&
        broadcast_increment_deriv!(a, output_deriv, bs_partials, a_bound, bs_partials_bound)
    istracked(b) &&
        broadcast_increment_deriv!(b, output_deriv, ex_partials, b_bound, ex_partials_bound)
    unseed!(output)
    return nothing
end
